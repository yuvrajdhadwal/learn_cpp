...
// The C++26 stdCode is a placeholder since the exact code won't be determined until the standard is
// finalized
...
long getCPPStandard() {
    // Visual Studio is non-conforming in support for __cplusplus (unless you set a specific
    // compiler flag, which you probably haven't) In Visual Studio 2015 or newer we can use
    // _MSVC_LANG instead See
    ...
    return -1;
#else
    // __cplusplus is the intended way to query the language standard code (as defined by the
    // language standards)
    return __cplusplus;
#endif
}
