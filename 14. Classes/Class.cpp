...
class Date {
   ...
   public:  // class members are private by default, any private member removes access to aggregate
    void printDate() { std::cout << m_month << '/' << m_day << '/' << m_year << '\n'; }
    ...
    void printTwoDates(const Date& o) {  // access level specifiers are per-class not per-object
        ...
        std::cout << m_month << '/' << m_day << '/' << m_year << ' ' << o.m_month << '/' << o.m_day
                  << '/' << o.m_year << '\n';
    }
   ...
   private:       // its still good to be explicit about public/private/protected
    ...
    std::string m_holidayName;  // has default constructor no need to directly init
};
...
int main() {
    ...
    // prefer static cast when performing narrowing conversion or
    // make it really obvious that we are converting the type or
    ...
    // prefer this when do not want to perform narrowing conversion or
